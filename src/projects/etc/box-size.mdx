...
해당 원리에 의하면 카메라와 박스간의 거리를 구하면 실제 박스 크기를 구할 수 있다는 걸 알 수 있습니다. **아래 수식은 실제 박스 최하단 꼭지점을 월드 좌표**로 구하는 공식입니다.
...
그리고 `OpenCV`를 사용하여 크롬한 박스 이미지에서 **배경제거**를 하였고 **윤곽선을 검출하여 각 꼭짓점을 계산**했습니다. 계산된 꼭짓점을 이용하여 상자의 꼭짓점을 `3D 좌표계로 변환`하였고, **캘리브레이션을 통해 계산된 파라미터를 활용**하여 실제 `월드 좌표계`로 변환하여 **실제 박스의 체적을 도출**해낼 수 있었습니다.
