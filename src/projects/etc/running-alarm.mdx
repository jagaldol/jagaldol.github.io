...
해당 프로젝트는 임베디드 보드(`STM32F107VCT6`)와 `다양한 센서들`을 활용하여 제작되었습니다. 1000페이지에 달하는 보드에 대한 레퍼런스가 주어졌고, 이를 해석하여 보드의 사용법, 더 나아가 많은 센서들과의 연결을 하였습니다. 또한, `Critical Section`을 통한 신호 전달로 각 센서들의 동작을 `동기화` 할 수 있었습니다.
